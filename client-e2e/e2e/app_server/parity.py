...
def text_rows(reference: Snapshot, other: Snapshot) -> list[tuple[int, str, str]]:
    """Return differing normalized rows as index and text pairs."""
    return _text_rows(_normalized(reference), _normalized(other))
...
def color_cells(
    ...
    """Return visible foreground and background differences."""
    return _color_cells(_normalized(reference), _normalized(other))
...
def captures_match(captures: Mapping[str, Sequence[Snapshot]]) -> bool:
    """Return whether every capture matches the first capture."""
    if not captures:
        return True
    snapshots = iter(captures.values())
    reference = next(snapshots)
    for other in snapshots:
        if len(reference) != len(other):
            return False
        if any(
            not snapshots_match(reference_snapshot, other_snapshot)
            for reference_snapshot, other_snapshot in zip(reference, other, strict=True)
        ):
            return False
    return True
...
def normalize_requests(requests: Sequence[Request]) -> list[Request]:
    ...
    return _coalesce_agent_switches([
        _normalize_request(request) for request in requests
    ])
...
def _normalize_request(request: Request) -> Request:
    return Request(request.method, _normalize_request_value(request.params))
...
def _normalized_cells(snapshot: Snapshot) -> list[list[Cell]]:
    scrollbar = any(_SCROLLBAR_THUMB.match(row[-1].character) for row in snapshot.cells)
    normalized: list[list[Cell]] = []
    for snapshot_row in snapshot.cells:
        cell_row = _normalize_vibe_dir(list(snapshot_row))
        grid_row = "".join(cell.character for cell in cell_row)
        pid_columns = _pid_columns(grid_row) | _elapsed_columns(grid_row)
        row: list[Cell] = []
        # Columns containing scrollbar thumb glyphs (popup or transcript).
        # Mask the full column so floating-point rounding differences in the
        # scrollbar position do not cause platform-dependent golden mismatches.
        scrollbar_cols = {
            x
            for x in range(len(cell_row))
            if _SCROLLBAR_THUMB.match(cell_row[x].character)
        }
        for x, cell in enumerate(cell_row):
            volatile = (
                ...
                or (x in scrollbar_cols and _SCROLLBAR_THUMB.match(cell.character))
            )
            if volatile:
