...
def save_svgs(snapshots: list[Snapshot], label: str, out_dir: Path) -> list[Path]:
    """Write one SVG per snapshot under out_dir; return the paths written."""
    out_dir.mkdir(parents=True, exist_ok=True)
    paths: list[Path] = []
    for i, snap in enumerate(snapshots):
        slug = "".join(ch if ch.isalnum() or ch in "_-" else "_" for ch in snap.label)
        path = out_dir / f"{label}_{i:02d}_{slug}.svg"
        path.write_text(to_svg(snap, f"{label} [{snap.label}]"))
        paths.append(path)
    return paths
