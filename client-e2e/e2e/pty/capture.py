...
def open_terminal(rows: int, columns: int) -> tuple[int, int]:
    """Open a correctly sized PTY pair for a launcher that does not fork here."""
...
def _wait_for_exit(pid: int, fd: int, failsafe: float) -> None:
    ...
    # A zygote-forked client is the zygote's child, never ours; reap ours, bounded.
    while True:
        try:
