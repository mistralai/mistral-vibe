...
# Known divergence outside this unit: rust keeps the transient
# "Resuming session…" local notice across the resume rebind while python's
# transcript rebuild drops it. The attachment rows themselves are pinned by
# the per-client assertions and the rust golden.
skip_terminal_parity = "rust keeps the transient 'Resuming session…' notice across the resume rebind, python drops it"
...
# Rust renders every row one above python's (the notice divergence above), so
# each client's `~/pics/cat.png` link is pinned by its own click on adjacent rows.
expected_actions = {
