...
- The classifier is permissive by default: it blocks only a concrete effect matching a deny rule or a dangerous action whose scope exceeds the user's authorization. A permission boundary, external location, unfamiliar command, or missing allow example is not independently dangerous.
