...
def test_resolve_marks_connector_exceeding_tool_limit_as_unavailable() -> None:
    ...
    by_alias = {connector.alias: connector for connector in catalog.connectors}
    assert set(by_alias.keys()) == {"many", "wiki"}
    assert not by_alias["many"].ready
    assert by_alias["many"].tools == ()
    assert by_alias["many"].diagnostics == (
        "Connector bootstrap issue: tool_limit_exceeded",
    )
    assert by_alias["wiki"].ready
