...
use crate::selection::{
    BottomBarSelection, ClickChain, Granularity, Region, TableCellHit, TableCellSelection,
};
...
pub enum Surface {
    /// The selectable region the current screen published (transcript, trust gate).
    Region,
    Composer,
...
pub struct Overlays {
    ...
    /// Active warning/error toast overlay (Python `App.notify`), self-hiding after its timeout.
...
impl App {
    ...
    /// Show a toast for `seconds` (Python `App.notify(timeout=...)`).
    pub fn show_toast(&mut self, text: String, severity: ToastSeverity, seconds: u64) {
        self.overlays.toast = Some(Toast {
            text,
            severity,
