...
pub struct Cli {
    ...
    /// Agent to use (builtin or custom from ~/.vibe/agents/NAME.toml).
    #[arg(long = "agent")]
    pub agent: Option<String>,

    /// Approve all tool calls without prompting (also --yolo).
    #[arg(long = "auto-approve", alias = "yolo", action = ArgAction::SetTrue)]
...
impl Cli {
    ...
    /// Build headless options from the parsed CLI + stdin prompt.
    ...
    /// interactive `SessionOptions` (cli.py): `--agent`, `--auto-approve` (with
    /// the `--yolo`-without-`--agent` selection), the tool filters, `--trust`,
    /// and `--add-dir`. Budget flags stay headless-only, as in Python.
    pub fn interactive_agent_config(&self, cwd: Option<String>) -> Result<AgentConfig> {
        let (agent, auto_approve) = match (&self.agent, self.auto_approve) {
            (Some(a), auto) => (Some(a.clone()), auto),
            (None, true) => (Some("auto-approve".into()), false),
