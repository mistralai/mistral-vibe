...
const COMMANDS: &[Command] = &[
    ...
    cmd(
        &["/stress"],
        "Repeat the /demo history entries to load-test rendering. Optional arg: entries (default 100).",
        true,
    ),
...
/// Vibe Code Web commands: shown only when `vibe_code_enabled`, matching Python's `is_available`.
const VIBE_CODE_COMMANDS: &[Command] = &[
...
/// Commands available in this context, gating Vibe Code Web on `vibe_code_enabled`
/// (Python `is_available`).
fn available(vibe_code_enabled: bool) -> impl Iterator<Item = &'static Command> {
    let vibe_code: &[Command] = if vibe_code_enabled {
        VIBE_CODE_COMMANDS
    } else {
        &[]
    };
    COMMANDS
        .iter()
        .chain(vibe_code.iter())
        .chain(PASTE_IMAGE.iter())
}

/// Whether the command runs while a turn is generating (Python `side_channel=True`).
pub fn is_side_channel(label: &str, vibe_code_enabled: bool) -> bool {
    available(vibe_code_enabled)
        .any(|command| command.aliases.contains(&label) && command.side_channel)
}

/// Resolve the slash-command word, returning the canonical alias.
pub fn parse(input: &str, vibe_code_enabled: bool) -> Option<&'static str> {
    ...
    available(vibe_code_enabled).find_map(|command| {
        command
            .aliases
            .iter()
...
pub fn builtin(vibe_code_enabled: bool) -> Vec<(String, String)> {
    let vibe_code: &[Command] = if vibe_code_enabled {
        VIBE_CODE_COMMANDS
    } else {
        &[]
...
pub fn entries(skills: &[(String, String)], vibe_code_enabled: bool) -> Vec<(String, String)> {
    let vibe_code: &[Command] = if vibe_code_enabled {
        VIBE_CODE_COMMANDS
    } else {
        &[]
