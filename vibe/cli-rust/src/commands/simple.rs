...
pub fn help_text(app: &App) -> String {
    ...
    let mut commands = super::builtin(app.session.startup_config.vibe_code_enabled);
    ...
    tokio::spawn(async move {
        ...
        let text = match result {
            Ok(value) => value
                .get("title")
                .and_then(serde_json::Value::as_str)
                .map(|title| format!("Session renamed to \"{title}\"."))
                .unwrap_or_else(|| "Failed to rename session: invalid response".to_owned()),
            Err(error) => format!("Failed to rename session: {error}"),
        };
        let _ = tx.try_send(CommandEvent::Result(text));
    });
}
