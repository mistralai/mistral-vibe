...
pub fn flush_pending(
    ...
    let vibe_code_enabled = app.session.startup_config.vibe_code_enabled;
    for value in pending {
        let Some(command) = parse(&value, vibe_code_enabled) else {
            continue;
        };
        // A queued prompt may have started a turn before deferred commands replay.
        if !is_side_channel(command, vibe_code_enabled) {
...
pub fn submit(
    ...
    let vibe_code_enabled = app.session.startup_config.vibe_code_enabled;
    let classified = classify(&value, &app.completion.skills, vibe_code_enabled);
