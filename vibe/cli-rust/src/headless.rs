...
async fn start_session(
    ...
    let disabled_tools = build_disabled_tools(&options.disabled_tools);
    // Python _agent_selection: --yolo without --agent → auto-approve agent, clear flag.
    let (agent, auto_approve) = match (&options.agent, options.auto_approve) {
