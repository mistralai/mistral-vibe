...
#[cfg(unix)]
pub(crate) fn request_suspend(app: &mut App, key: &KeyEvent) -> bool {
...
#[cfg(not(unix))]
pub(crate) fn request_suspend(_: &mut App, _: &KeyEvent) -> bool {
    false
}
...
pub fn handle_priority_key(app: &mut App, client: &Arc<Client>, key: KeyEvent) -> Option<bool> {
    ...
    if is_copy_key(&key) {
        ...
        if let Some(text) = chat_input::selected_text(
            &app.chat_input.input,
            app.chat_input.cursor,
            app.chat_input.anchor,
        ) {
            clipboard::copy_to_clipboard(&text);
        } else if let Some(text) = selection_text(app) {
...
/// Cut the selection: Ctrl+X (Textual TextArea `cut`).
...
pub fn handle_key(
    ...
    // Cut the chat input selection; copy is an app-level priority binding.
    if is_cut_key(&key) {
        if let Some(text) = chat_input::selected_text(
            &app.chat_input.input,
            app.chat_input.cursor,
            ...
            chat_input::apply(
                &chat_input::Action::DeleteSelection,
