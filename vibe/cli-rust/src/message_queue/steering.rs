...
use super::{replacement, QueueEvent};
...
pub fn mutation_in_flight(app: &App) -> bool {
    app.queue.replacing.is_some() || is_active(app)
}
...
pub fn steer_pending(app: &mut App, client: &Arc<Client>) -> bool {
    ...
    if app.queue.replacing.is_some() || app.queue.items.iter().any(|item| !item.sent) {
        app.queue.steer_after_replace = true;
        if app.queue.replacing.is_none() {
...
fn finish(app: &mut App, client: &Arc<Client>, server_message_id: &str) {
    ...
    while let Some(index) = app
        ...
        .items
        .iter()
        .position(|item| item.server_message_id == server_message_id)
    {
