...
pub fn resolve(
    ...
    let rows: Vec<Row> = flow
        .into_iter()
        .filter_map(|span| body(buf, chat, span, gutter_at(gutters, span.0), copy_blank_rows))
        .collect();
...
fn body(
    ...
    let content_start = chat.x.saturating_add(BODY_PADDING).min(row_end);
    ...
    if visible.iter().all(|(_, symbol)| *symbol == RULE) {
        return None;
    }
    let mut lo = first_x.min(content_start);
    if CHROME_GLYPHS.contains(&first_symbol) && is_blank(buf, first_x + 1, y) {
