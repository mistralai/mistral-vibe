...
pub fn press(app: &mut App, at: (u16, u16)) {
    press_with_padding(app, at, false);
}
...
pub fn press_including_padding(app: &mut App, at: (u16, u16)) {
    press_with_padding(app, at, true);
}

fn press_with_padding(app: &mut App, at: (u16, u16), include_padding: bool) {
    ...
    let selectable =
        region::selectable(app, at) || (include_padding && app.view.selection_region.contains(at));
    if selectable {
        app.chat_input.anchor = None;
        app.selection.drag = Some(Surface::Region);
        let point = app.view.selection_region.point(at);
        let table_cell = table::selection_at(app, at);
...
pub fn drag(app: &mut App, at: (u16, u16)) {
    ...
    match app.selection.drag {
        ...
        Some(Surface::Region) => {
            let head = app.view.selection_region.point(at);
            let in_table = app
                .selection
                .region
                .as_ref()
            ...
            let edge_scroll = edge_row.map_or(0, |row| {
...
pub fn release(app: &mut App) -> Release {
    ...
    match surface {
        ...
        Some(Surface::Region) => match app.selection.region.as_mut() {
            Some(selection)
