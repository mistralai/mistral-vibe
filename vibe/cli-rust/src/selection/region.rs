...
pub fn extract_document(app: &App) -> Option<String> {
    ...
    if !app.view.selection_region.document {
        return None;
    }
    ...
    let spans = resolve(
        Flow {
            selection: (selection.anchor, selection.head),
            origin: 0,
            end_exclusive: app.view.selection_region.end_exclusive,
        },
        app.selection.granularity,
        &document.buffer,
        document.area,
...
/// cell some widget owns, since Textual anchors nothing on bare padding.
pub fn selectable(app: &App, at: (u16, u16)) -> bool {
    app.view.selection_region.contains(at)
        && !app
            .view
            .selection_chrome
            .iter()
