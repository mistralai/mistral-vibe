...
impl Transcript {
    ...
    fn insert(&mut self, mut raw: Value, historical: bool) -> bool {
        ...
        self.next_rev += 1;
        let typed = HistoryEntry::from_value(&raw);
        let groups = groups_with_tools(&typed);
    ...
    fn group_at<'a>(&'a self, index: usize, stored: &'a StoredEntry) -> Option<Group<'a>> {
        ...
        let start = self.entries[..index]
            .iter()
            .rposition(|entry| !groups_with_tools(&entry.typed))
            .map_or(0, |boundary| boundary + 1);
        let end = self.entries[index + 1..]
            .iter()
            .position(|entry| !groups_with_tools(&entry.typed))
    ...
    pub fn is_expandable(&self, id: &str) -> bool {
            ...
            .map(|stored| &stored.typed)
        {
            ...
            Some(HistoryEntry::Effect(effect)) => effect.is_collapsible() && effect.has_body(),
            Some(HistoryEntry::Reasoning(_)) => true,
            _ => false,
        }
    }
