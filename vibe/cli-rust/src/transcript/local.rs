...
/// Promote a queued prompt to a started one (Python `UserMessage.set_pending`).
pub fn clear_pending(transcript: &mut Transcript, id: &str) {
    transcript.patch_local(id, |entry| {
        if let Some(entry) = entry.get_mut("pending") {
