...
pub fn vertical_cursor(app: &App, down: bool) -> (usize, bool) {
    ...
    let cursor = crate::chat_input::clamp_offset(input, app.chat_input.cursor);
    let (marker, body, prefix) = match input.strip_prefix('/') {
        Some(rest) => ("/ ", rest, 1),
        None => ("> ", input.as_str(), 0),
    };
    let body_cursor = crate::chat_input::clamp_offset(body, cursor.saturating_sub(prefix));
    let layout = ComposerLayout::new(body, marker, body_cursor, app.view.input_area.width);
    let (offset, moved_row) = layout.vertical_offset(down);
    (prefix + offset, moved_row)
}
