...
use super::{bottom_bar, loading, scrollbar, theme, toast, transcript};
use crate::app::App;
...
pub fn draw(app: &mut App, f: &mut Frame, area: Rect) {
    ...
    // Textual's `ToastRack` keeps the input box's slot even while a bottom-app
    // replaces it, so toasts float over the browser at their usual height.
    let composer = super::input_box_height(app, area.width, area.height);
    let slot = Rect::new(area.x, area.bottom().saturating_sub(1 + composer), 0, 0);
    toast::draw(app, f, slot);
}
