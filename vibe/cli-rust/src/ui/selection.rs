...
use crate::app::App;
use crate::selection::region::{self, RowSpan};

/// Highlight the selection inside whatever region the current screen published.
pub fn overlay(app: &mut App, f: &mut Frame) {
    if app.selection.region.is_none() {
        return;
    }
    // The scrollbar gutter is chrome; selection stops one column short of it.
    let content_chat = app.view.selection_region.content();
