...
fn accepted_replacement_commits_the_new_text_and_images() {
    ...
    app.queue.items[0].replacement_pending = true;
    ...
    message_queue::apply_event(
        ...
        message_queue::QueueEvent::GroupReplaced {
            server_message_id: "message-1".to_owned(),
            message_ids: vec!["message-1".to_owned()],
            images,
            error: None,
        },
    );
    ...
    assert_eq!(app.queue.items[0].images.len(), 1);
    assert!(!app.queue.items[0].replacement_pending);
}
...
fn merged_replacement_restores_images_to_their_visible_prompts() {
    ...
    let images = ["first.png", "second.png"]
        .into_iter()
    ...
    message_queue::apply_event(
        ...
        message_queue::QueueEvent::GroupReplaced {
            server_message_id: "message-1".to_owned(),
            message_ids: vec!["message-1".to_owned(), "message-2".to_owned()],
            images,
            error: None,
        },
    );
    ...
    assert!(app.queue.items.iter().all(|item| !item.replacement_pending));
}
...
fn a_failed_replacement_allows_the_prompt_to_be_edited_again() {
    ...
    message_queue::apply_event(
        ...
        message_queue::QueueEvent::GroupReplaced {
            ...
            message_ids: vec!["message-1".to_owned()],
            images: Vec::new(),
            error: Some("replace failed".to_owned()),
        },
    );
    message_queue::edit_selected(&mut app);
