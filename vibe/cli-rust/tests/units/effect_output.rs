...
fn file_output_is_sanitized_before_stripping_read_line_numbers() {
    ...
    assert_eq!(
        format_effect_output(Some("file_read"), Some(&output)),
        ["first", "\tsecond"]
    );
    assert_eq!(
        format_effect_output(Some("file_write"), Some(&output)),
        ["   1→first", "   2→\tsecond"]
    );
    for kind in ["file_read", "file_write"] {
        let controls = json!({"content": "\x1b[2J\x1b[H\x07\x00"});
        assert!(format_effect_output(Some(kind), Some(&controls)).is_empty());
        assert!(format_effect_output(Some(kind), Some(&json!({}))).is_empty());
    }
}
