...
#[test]
fn unicode_graphemes_move_and_delete_as_one_character() {
    ...
    let (mut input, mut cursor, mut anchor) = state("a👩‍💻b", "a👩‍💻".len());
    apply(&Action::CursorLeft, &mut input, &mut cursor, &mut anchor);
    assert_eq!(cursor, 1);
    apply(&Action::DeleteRight, &mut input, &mut cursor, &mut anchor);
    assert_eq!(input, "ab");

    let (mut input, mut cursor, mut anchor) = state("ae\u{301}", "ae\u{301}".len());
    apply(&Action::DeleteLeft, &mut input, &mut cursor, &mut anchor);
    assert_eq!((input.as_str(), cursor), ("a", 1));
}
