...
use vibe_rs::selection::{
    self, region, Granularity, RegionId, ScrollTarget, TableCellHit, TableCellKey, TableCellSpan,
};
...
fn selected_as(row: &str, from: u16, to: u16, granularity: Granularity) -> Vec<(u16, u16, u16)> {
    selected_with_endpoint(row, from, to, granularity, false)
}

fn selected_with_endpoint(
    row: &str,
    from: u16,
    to: u16,
    ...
    selected_owner(row, from, to, granularity, RegionId::Main, end_exclusive)
}

fn selected_toast(row: &str, from: u16, to: u16) -> Vec<(u16, u16, u16)> {
    selected_owner(row, from, to, Granularity::Char, RegionId::Toast(1), false)
}
...
#[test]
fn frame_regions_select_without_chrome_gaps() {
    let question = selected_owner(
        "Which color?",
        2,
        9,
        Granularity::Char,
...
#[test]
fn double_click_expands_an_exclusive_endpoint_to_the_word() {
    assert_eq!(
        selected_with_endpoint("select this word", 8, 8, Granularity::Word, true),
        vec![(0, 7, 10)]
    );
}
...
#[test]
fn multiple_chrome_gaps_preserve_every_selected_segment() {
    let area = Rect::new(0, 0, 20, 1);
    let mut buffer = Buffer::empty(area);
    buffer.set_string(0, 0, "first  second  third", Style::default());
    for gaps in [vec![(0, 5, 6), (0, 13, 14)], vec![(0, 13, 14), (0, 5, 6)]] {
        let mut app = App::default();
        app.view.selection_region.area = area;
        app.view.selection_chrome = gaps;
        selection::press(&mut app, (0, 0));
        selection::drag(&mut app, (19, 0));

        let spans = region::spans(&app, &buffer, area);
        assert_eq!(spans, vec![(0, 0, 4), (0, 7, 12), (0, 15, 19)]);
        assert_eq!(region::extract(&buffer, &spans), "first\nsecond\nthird");
    }
}
...
fn table_selection_autoscroll_uses_the_clamped_cell_head() {
    ...
    let mut app = App::default();
    app.view.selection_region.area = chat;
    app.view.selection_region.scroll_target = ScrollTarget::Transcript;
    app.view.selection_region.scroll_area = chat;
    app.view
        .selection_scrollbar
