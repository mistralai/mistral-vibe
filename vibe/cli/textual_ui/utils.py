...
def load_conversation_from_file(filepath: Path, agent) -> tuple[dict, list]:
    """
...
def build_conversation_tree(conv_dir: Path) -> list[ConversationTreeItem]:
    """
    Build a tree structure of conversations and folders from the conversations directory.
    
    Args:
        conv_dir: Path to the conversations directory
    ...
    for conv_file in conv_files:
        try:
            ...
            # Get timestamp
            # saved_at could be a relative timestamp or absolute timestamp
