...
| `PluginManager` | `plugins/manager.py` | Plugin loader with pluggy |
| `VibePlugin` | `plugins/base.py` | Plugin base class |
| `SkillManager` | `skills/manager.py` | Skill loader |

## CONVENTIONS

- Inherits all root conventions
- Tools: subclass `Tool`, implement `run()`
- LLM backends: subclass `LLMBackend`, implement `complete()`
- Plugins: subclass `VibePlugin` from `plugins/base.py`
...
- No plugin loading failures - graceful degradation with circuit breaker
...
- Plugin resilience via pybreaker (3 failures threshold, 30s recovery)
