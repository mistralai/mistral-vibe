...
def _get_windows_system_prompt(bash_config: BashToolConfig | None = None) -> str:
    ...
    if shell_executable and "sh" in Path(shell_executable).name:
        rules = (
            f"- {shell_executable} is available ('Git binaries / Mingw64 / Cygwin') — Unix commands like `ls`, `grep`, `cat` work\n"
            "- **Use forward slashes (`/`) in paths** for bash\n"
